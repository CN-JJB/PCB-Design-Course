...
## 7.2 用 Nordic 官方资料校验：Antenna Keepout 和 RF Feed Reference 不是一回事

上一节的 Reddit wearable 案例很有价值，但它只能说明一个真实设计怎么做，不能单独作为 RF 规则来源。

Nordic 的官方 nRF52 PCB 指南提供了更重要的区分：

### ① RF Feed 段需要稳定的参考结构

从 nRF RF pin / center pad 到 matching network / antenna feed 的传输路径，需要按参考设计维持：

- 明确的 50 Ω 结构；
- 连续的近邻 GND reference；
- RF path 两侧合适的 GND / via stitching；
- 不让数字/电源网络从 RF path 下方穿过。

### ② Antenna Radiating Area 又可能要求多层 Keepout
...
| 区域 | 主要目标 | Ground 处理 |
|---|---|---|
| RF feed / matching | 让传输线阻抗和回流稳定 | 需要连续、可预测的参考结构 |
| antenna radiating keepout | 不让附近铜改变天线场和调谐 | 按 vendor 文档让开铜层 |
...
正确做法是先把 **feed region** 和 **radiating region** 分开，再逐区执行器件厂商的 layout requirement。

### 一手资料

- Nordic Semiconductor, *General PCB design guidelines for nRF52 series*  
  https://devzone.nordicsemi.com/guides/hardware-design-test-and-measuring/b/nrf5x/posts/general-pcb-design-guidelines-for-nrf52-series
- Nordic Semiconductor, *Advice for Designing with the nRF52*  
  https://devzone.nordicsemi.com/nordic/nordic-blog/b/blog/posts/advice-for-designing-with-thenrf52

> Reddit 负责提供“真实板怎么踩坑”的现场语境；Nordic 文档负责定义这个器件真正允许的 RF 几何。
