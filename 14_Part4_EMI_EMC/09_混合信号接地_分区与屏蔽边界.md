...
## 9.6.2 一手资料三角校验：Ott、TI、ADI 对 AGND/DGND 实际上有多一致？

上一节是论坛里的工程争议。这一节换成一手/高质量资料，看它们有没有真正支持“默认切地”。

答案反而相当一致。

### Henry Ott

核心不是“画 AGND / DGND 两块铜”，而是：
...
> **AGND / DGND 是芯片内部功能命名，不自动等于 PCB 上必须有两个完全分开的 ground plane。**

对低数字电流 converter，AGND/DGND 通常应以最小引线长度接到同一个低阻抗 reference。

对数字电流很大的特殊 converter，才可能讨论分区 plane、ferrite / Schottky、特殊 bridge，以及对应的验证。

### ADI MT-031

ADI 的重点与 TI 类似：

> DGND 表示 die 内部 digital ground connection，不等于“PCB 上把这个 pin 拉到数字地岛”。

对 low-digital-current ADC/DAC，通常把器件整体当成 analog device，AGND/DGND 以低阻抗方式进入 analog reference environment。

### 🧩 为什么 Eval Board 的“星形地”经常不能原样复制？

单颗 ADC eval board 很容易画成一个 converter-centered star point，但产品板有多颗 ADC/DAC、MCU、电源输入、connector 和多个 crossing。

如果每颗 converter 都各做一个“唯一 star”，系统会出现多个闭合 loop 与不可控 return。

> **Eval board 是单器件验证环境，不等于多器件产品的 system grounding architecture。**

### 🎓 一个非常重要的术语练习

看到 datasheet 写 AGND / DGND / PGND，先不要立即创建三个 PCB net。先填表：

| Pin name | Die/internal function | Recommended PCB connection in datasheet | Local current loop | System reference |
|---|---|---|---|---|
| AGND | | | | |
| DGND | | | | |
| PGND | | | | |

这能强迫你区分：**“引脚名”与“系统地架构”是两个层级。**

### 一手/高质量来源

- Henry Ott, *Grounding of Mixed Signal PCBs*  
  https://hott.shielddigitaldesign.com/techtips/split-gnd-plane.html
- TI SLYT499, *Grounding in mixed-signal systems demystified, Part 1*  
  https://www.ti.com/lit/an/slyt499/slyt499.pdf
- TI SLYT512, *Grounding in mixed-signal systems demystified, Part 2*  
...
## 9.6.3 Reddit 现场课：为什么“不要分地”和“必须分地”可以同时成立？

这批 Reddit 实际抓到的帖子正好形成一组很好的对照。

### Case A：普通 Mixed-Signal MCU / ADC

讨论 `Analog and Digital Power and Ground Lines` 时，多位参与者主张：

- common solid GND；
- analog / digital 通过 placement 与 routing 分区；
- 如果需要，**power rail 可以局部分开或滤波**；
- 不要先把 GND plane 切成岛。
...
另一个 review 讨论的是 **primary / secondary isolation barrier**。这里 primary GND 与 secondary GND 本来就不能做成同一块连续铜。

原因不是“模拟地更干净”，而是：

> **galvanic isolation / creepage / clearance / safety insulation。**

### 所以“Split Ground”必须先分类

| 场景 | 默认判断 | 核心原因 |
|---|---|---|
| MCU + ADC/DAC mixed signal | 通常不切 GND plane | return continuity / shared reference |
| 多 rail power plane | 可以按 rail 分铜，但高速线不能跨 split | DC power distribution |
| primary / secondary isolated PSU | 必须保持隔离边界 | safety / galvanic isolation |

如果不先做这个分类，就会出现最危险的知识迁移：

> “电源适配器都分地，所以 ADC 也该把 AGND/DGND 切开。”

这是完全不同的问题。

### Reddit 资料的定位

Reddit 不是规范，但它很适合做 **Fault Lab 的真实语料**：让学生看见同一个词 `split ground` 在不同上下文里其实代表三件完全不同的工程动作。

### 工程现场来源

- Reddit r/PCB, *Analog and Digital Power and Ground Lines*  
  https://www.reddit.com/r/PCB/comments/1v4pqof/analog_and_digital_power_and_ground_lines/
- Reddit r/PrintedCircuitBoard, *USB Wall Adapter PCB Review — Primary/Secondary Isolation Barrier...*  
  本批资料只取得镜像搜索片段与评论内容，未取得可核验 permalink，因此课程不把它当正式规范引用。
