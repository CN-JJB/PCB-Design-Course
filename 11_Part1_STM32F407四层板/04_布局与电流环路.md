...
### 7.1 晶振布局争议卡：Guard Ring、Local GND、完整 Plane 到底谁对？

晶振是另一类“不同 appnote / 工程师会给出不同画法”的典型区域。

你可能看到：

- 晶体周围画 GND guard ring；
- OSC_IN / OSC_OUT 之间插 GND；
- 晶体下方铺连续 GND；
- MCU + 晶体做 local GND island，再单点接主 GND；
- 禁止任何 GND 切割，只要求 loop 小。

不要把这些画法当成互相独立的 checklist。先问它们各自在解决什么：

| 做法 | 想解决的问题 | 可能的副作用 |
|---|---|---|
| 缩短 OSC loop | 降低环路面积、寄生、外部耦合 | 几乎总是首要 |
| 连续参考铜 | 提供稳定屏蔽/return environment | 若为追求“隔离”而切 plane，可能制造 slot |
| Guard ring | 局部屏蔽 / 隔离直觉 | 如果形成长而悬空/单点连接结构，反而可能成为耦合导体 |
| Local GND island | 控制 MCU/oscillator 高频电流局部闭合 | island/bridge 设计不好时可能制造新的 reference discontinuity |
...
~~~text
1. 晶体与 load caps 靠近 oscillator pins
2. OSC_IN / OSC_OUT 短、彼此不过度耦合
3. 不让 SWCLK / switch node / LED 大电流路径穿过
4. 下方保持可解释的连续 reference
5. 不挂长 test stub
6. 只有在器件 appnote / 测量证据支持时再增加 guard / island 特殊结构
~~~

也就是说：

> **先把“小回路 + 好分区 + 连续 reference”做好，再讨论装饰性很强的 guard ring。**

### 🎮 Fault Lab：哪个“保护晶振”的动作反而值得怀疑？
...
外面绕一圈细长 GND guard
guard 只有一个远端 via
为了 guard 又把下方 GND 挖出一圈 clearance
~~~

B 看起来“更高级”，但必须重新证明它没有把 reference / coupling 做坏。
...
- Electronics StackExchange, *Competing PCB Crystal layout recommendations*  
