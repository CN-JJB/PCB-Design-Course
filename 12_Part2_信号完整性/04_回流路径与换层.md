...
### 4.2.2 从“口诀”到“实测”：Bogatin 的 Return Current 演示怎么读？

资料集里收录了 Eric Bogatin 对 Bruce Archambeault 经典实验/仿真的复现。它很适合用来纠正一句过度简化的话：
...
- 很慢的变化中，电阻对电流分布影响更明显；
- 随着频率上升，环路电感越来越重要；
- 对紧邻 plane 的传输线，快速分量会越来越集中在 trace 投影附近。

资料中的特定实验给出了大致趋势：

| 观察频率/边沿 | 实验现象 | 应该学到什么 |
|---|---|---|
| kHz 量级 | 回流还明显分散到更低电阻路径 | 低频/音频不能机械套“全部贴线” |
| 数十 kHz 往上 | 邻近 trace 的回流比例迅速增大 | loop inductance 开始主导 |
| MHz 量级 | 绝大部分回流已集中在邻近 reference | 快速数字线需要连续 reference |

这些频率是**该实验几何的结果**，不能升级成：
...
给学生一个 U 形 signal path，再提供一条“电阻更短、但回路面积更大”的 return shortcut。

先问：

1. DC 时你预测 return 主要在哪里？
2. 频率上升后哪条路径的 loop inductance 更小？
3. 如果把 signal-reference 间距 H 缩小，crossover 会怎样变化？
4. 如果在 reference 上切一道 slot，原本的“贴线回流”会发生什么？

最后再打开原文/实验结果核对。

### 为什么这个实验对 Mixed-Signal 特别重要？

它同时说明两件看似矛盾的事：

- **快速 digital return** 通常不会无缘无故跑进很远的 analog 区；
- **低频/精密 DC return** 的分布却可能更宽，更容易通过共享铜产生 µV 级误差。

这就是为什么本课程在 Part 4 同时强调：

~~~text
高速问题 → reference continuity / loop geometry
低频精密问题 → shared impedance / IR drop / current entry-exit
~~~

### 来源

- Eric Bogatin, *A Simple Demonstration of Where Return Current Flows*  
...
### Reference plane 的 DC 电压不是核心条件

一个数字信号可以把连续的 PWR plane 当作 AC reference，即使该 plane 的 DC 电压并不等于信号逻辑电压。

真正要问的是：

- reference 是否连续；
- signal-reference geometry 是否稳定；
- 是否跨 split / void；
- 换层后 return current 怎样从 old reference 转到 new reference。

因此“3.3 V signal 必须参考 3.3 V plane”不是通用规则。

但如果信号从参考 GND 的层换到参考 PWR 的层，return current 就必须通过附近的 GND↔PWR 高频 coupling / decoupling path 完成 reference transition；若两个 reference 都是 GND，则 nearby stitching via 通常更直接。

进一步见 Part 3：[电源层真的必要吗：双 GND、Reference Transition 与 Plane Cavity](../13_Part3_电源完整性/12_电源层真的必要吗_双地平面与PlaneCavity.md)。
...
### 4.6.1 “Via 越小，电感越低”要拆成 Inductance 与 Capacitance 两个问题

视频建议高速 signal via 尽量使用小 via，并把理由概括成“小 via 电感更低”。课程需要纠正这句话。

对单个 through via 的一阶 self-inductance，常见近似形式包含：

Lvia ∝ h × [ln(4h/d) + ...]

其中 h 是 via length，d 是 barrel diameter。

这说明：

- via 越短，通常越能明显降低电感；
- diameter 对 inductance 的影响是对数级；
- 其他条件相同时，更大的 diameter 往往会略降低 self-inductance；
- 多个合理靠近的 return vias 可以降低局部 loop / shared inductance。

高速设计采用较小 signal via 时，真正可能得到的收益常包括：

- pad / antipad 更小；
- parasitic capacitance 可能降低；
- reference plane 被打掉的面积更小；
- BGA escape / return-via placement 更容易；
- transition 可以做得更紧凑。
...
> 这个 via transition 的 barrel length、pad/antipad、stub、reference-via geometry 和制造能力共同产生什么阻抗不连续？

TI 的 PCB layout guideline 也明确指出，via diameter 对 inductance 的影响相对有限，显著降低 inductance 更依赖缩短 via length 或合理使用并联 vias。

### 4.6.2 SIG / GND / GND / SIG 外层到外层“无 Via Stub”是什么意思？
...
中，如果 signal through-via 从 L1 一直连接到 L4，则没有超出连接终点的未使用 barrel 段，所以传统意义上的 dangling via stub 可以为零。

这个判断在该结构里成立。

但：

> 没有 unused barrel stub ≠ via transition 是理想 0 Ω。

仍然存在：

- barrel inductance；
- pad / antipad capacitance；
- reference-plane void；
- P/N asymmetry（差分时）；
- return-current transition。

如果 signal 只从 L1 连接到中间层，而 through-via 继续延伸到另一侧，就仍可能留下 unused stub。
...
而不是只根据“这是四层板”判断有没有 stub。


### 4.6.3 别只盯 Signal Via Stub：Power Via Stub 也可能耦合进高速通道

高速设计里大家都知道 signal via 的 dangling stub 会谐振，但 Istvan Novak 的案例提醒了一个更隐蔽的问题：

> **附近的 power via stub 自己虽然不承载高速数据，也可能在谐振时把场耦合进敏感 signal via / pair。**

资料里的 4 层示例用一个较长 through-via 说明：

- naive quarter-wave 估算得到一个很高的谐振频率；
- 加上 pad / antipad loading 后，真实 notch 会显著降低；
- 该 resonant power-via structure 会在邻近 channel 上产生 crosstalk / ringing。
...
~~~text
高速 transition 周围有哪些 long via structures？
它们的 pad/antipad/cavity 会不会形成耦合谐振？
~~~

### 🔬 Near-via Environment Review

对每个关键 SerDes / USB3 / fast clock via cluster，圈一个局部区域，检查：

- signal via stub；
- nearby power via stub；
- return via distribution；
- antipad overlap；
- plane perforation；
- connector launch / BGA escape。

到了多 GHz 通道，这个“via 周边环境”比单独数 signal-via 数量更接近真实问题。

### 来源

- Istvan Novak, *The Signal Integrity Perils of Power Via Stubs*  
  https://www.signalintegrityjournal.com/blogs/12-fundamentals/post/795-the-signal-integrity-perils-of-power-via-stubs
