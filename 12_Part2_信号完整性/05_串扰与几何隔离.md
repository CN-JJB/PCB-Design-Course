...
### 5.4.2 一个 Stackup A/B：S 不变，只把 H 从 63 mil 拉到 7 mil

Robert Feranec 的 IBIS / coupled-line 教学案例把两条 12 mil trace 保持 12 mil spacing，只改变 signal-reference height：

- 原始 H = 63 mil ≈ 1.60 mm；
- 改进 H = 7 mil ≈ 0.178 mm。

在该特定仿真里，victim crosstalk 明显下降。

这个例子直接说明：

> **“两条线间距没变”并不意味着 coupling environment 没变。**

H 大幅减小时，场更集中到 signal-reference 之间，邻线进入 fringe field 的比例下降。

同时该 stackup 变化也改变了 Z0，所以不要把“crosstalk 改善”误解成只有一个参数在变。完整 ablation 与 source termination / branch topology 的组合见：[11｜仿真案例：支路、源端串联终端与 Stackup](11_仿真案例_支路终端与Stackup.md)。
...
### 5.5.10 “3H”可以做 Routing 起点，但不是 Noise Budget

视频给出一个常见 spacing rule：相邻高速线之间尽量保持约 3H 或更多，其中 H 是 signal 到 reference plane 的高度。

这个规则的物理直觉有价值：

~~~text
S/H 增大
→ aggressor-victim mutual coupling 相对下降
→ crosstalk tendency 通常下降
~~~
...
1. 用 S/H 做第一轮 floorplan / routing spacing；
2. 对 clock、SerDes、memory、sensitive victim 建 noise budget；
3. 长并行段用 coupled-line solver / IBIS / channel simulation；
4. 只在真正需要的位置花更多 board area。

所以 3H 是 screening geometry，不是 universal pass/fail boundary。
...
## 5.9.1 规则冲突卡：3W、5W、5S、4–5H 到底该听谁？

查高速布线资料时，你会看到很多 spacing 口诀：

- 3W；
- 5W；
- 5S；
- 3H；
- 4–5 × reference-plane height。

如果把这些都当成“规范”，同一块板会得到五个互相冲突的间距。

更成熟的处理方法是先问：**这些规则用什么量去近似场耦合？**

| 规则 | 归一化变量 | 更适合表达什么 |
|---|---|---|
| 3W / 5W | trace width W | 快速 layout heuristic，没直接包含 stackup H |
| 5S | intra-pair spacing S | differential pair-to-pair isolation 的一种经验表达 |
| 3H / 5H | trace-to-reference height H | 更直接反映 field spread 与 stackup |

Peterson 的 differential-crosstalk 文章和 TI PCIe Gen5 指南都把 **height to reference plane** 作为重要尺度。原因很直观：

~~~text
H 小
→ field 更集中在 signal-reference 之间
→ 同样绝对间距下，邻线看到的 fringe field 更少
~~~

所以两块 trace width 都是 0.15 mm 的板：

- 4L，H = 0.21 mm；
- 6L，H = 0.076 mm；

不能只因为 W 一样，就认为需要一样的 pair-to-pair spacing。

### 🎮 设计题：同样 20 mil 间距，哪块更危险？
...
> **口诀只用来给第一版 routing envelope；最终 spacing 要由 H、coupled length、edge 和 noise budget 解释。**

### 来源

- Zachariah Peterson, *Differential Crosstalk and Differential Pair Spacing*  
  https://resources.altium.com/p/differential-crosstalk-and-spacing-between-differential-pairs
